...
public class CustomLookupBackup extends AbstractLookup {
	...
	for (Component comp : grid.getOwnComponents()) {
	    ...
	    if(pos != null) {
		if(pos.getColumn1() == 0) {
		    height += comp.getHeight() + 10; //allow for space between rows
		}
		if(pos.getRow1() == 0) {
		    width += comp.getWidth();
		}
...
//	    return;
	...
	for (int i = 0; i < entityArray.length; i++) {
		...
		if (sec != null) {
			...
			if (!val.equals(sec.currentValue)) {
			    ...
			    fillSection(sec, entity, "groupheader");
		...
		if (sec != null) {
			...
			if (i == entityArray.length - 1) {
			    ...
			    String val = null;
			    Method m = getClass().getMethod("getBreakValue" + sec.groupNumber, Entity.class);
			    if (sec.currentValue == null)
				sec.currentValue = (String) m.invoke(this, entityArray[i]);
			    val = (String) m.invoke(this, entityArray[i + 1]);
			    if (!val.equals(sec.currentValue)) {
				addFooter = true;
			    }
			    sec.currentValue = val;
			...
			if (addFooter) {
			    didAddFooter = true;
			    fillSection(sec, entity, "groupfooter");
			    sec.rowCountInGroup = 0;
			}
		    } catch (Exception e) {
			e.printStackTrace();
	...
	//System.out.println("FILL SECTION " + type+", "+sectionInfo.componentInfos.size());
	if(sectionInfo.componentInfos.size() == 0) {
	    //if the detail section is empty, still need to collect fields from the entity
	...
	if (type.equals("reportheader") || type.equals("reportfooter")) {
		...
		boolean useLastPart = numColsInPage > 1; //TODO: fix this better
		if (nextHeight > availablePageHeight) {
		    newPage(entity, useLastPart);
		}
		if ((numColsInPage > 1 && type.equals("groupheader")) || columnCount == 0) {
	...
	for (int i = 0; i < sectionInfo.componentInfos.size(); i++) {
		...
		if (comp instanceof Field) {
		    ...
		    for (Collector col : collectors.values()) {
			//if there is no field with the given name, see if the entity has a field of the same name
...
//		    ((WebLabel) wtf).setAlignment( ((Label) comp).getAlignment());
...
//			System.out.println("ADD COMP " + wtf + ", " + area.getColumn1() + ", " + area.getRow1());
		    } else {
			layout.add(wtf);
...
//		    System.out.println("Error trying to get "+fieldName+" from "+currentEntity+", "+ex);
		}
	    }
	}
	return val;
	...
	//TODO: clearly this isn't right, but this is used in a report and so we gotta do something else
	...
	if (source instanceof GroupDatasource) {
	    return ((GroupDatasource) source).getItems().size();
	}
